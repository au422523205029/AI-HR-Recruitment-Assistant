...
if not os.getenv("GOOGLE_API_KEY") and not os.getenv("GEMINI_API_KEY"):
    st.warning("Add GOOGLE_API_KEY (or GEMINI_API_KEY) to Streamlit Secrets before screening.")

job = st.text_area("Job Description", height=240, placeholder="Example: Full Stack Developer with Python, React, REST APIs...")
...
if st.button("🚀 Screen Candidates", type="primary", use_container_width=True):
    ...
    for f in files:
        ...
        if text:
            resumes[candidate_name_from_filename(f.name)] = text
    ...
    try:
        ...
        st.error("Gemini API request failed. Check your Google API key and free-tier quota.")
        st.exception(exc)
...
if st.session_state.results:
    ...
    for i, r in enumerate(ranked, 1):
        with st.container(border=True):
            ...
            a.metric("Rank", i)
            b.markdown(f"### {r.candidate_name}")
            b.write(r.summary)
            c.metric("Fit", f"{r.overall_score:.0f}/100")
            c.write(f"**{r.recommendation}**")
    ...
    if "questions" in st.session_state:
        ...
        for item in q.evaluation_focus:
            st.write("• " + item)

st.divider()
st.caption("Human review is required for employment decisions. This demo does not use protected characteristics.")
